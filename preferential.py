...
class PreferentialAttachmentGrowth(Scene):
    def construct(self):
        ...
        G = nx.barabasi_albert_graph(initial_nodes, m_edges)
        ...
        for node in G.nodes():
            x, y = pos[node]
            circle = Circle(radius=0.1).move_to(np.array([x, y, 0]))
        ...
        for new_node in range(initial_nodes, total_nodes):
            G.add_node(new_node)
            targets = list(G.nodes())[:new_node]  # Potential targets for connection
            ...
            pos[new_node] = (x, y)
            new_circle = Circle(radius=0.1).move_to(np.array([x, y, 0]))
            new_circle.set_fill(RED, opacity=0.8)
            node_circles[new_node] = new_circle
            
            # Animate node appearance
            self.play(FadeIn(new_circle), run_time=0.3)
