...
class PreferentialAttachmentGrowth(ThreeDScene):
    def construct(self):
        ...
        for node in G.nodes():
            x, y, z = pos[node]
            circle = Circle(radius=0.1).move_to([x * 3, y * 3, z * 3])  # Scaling for better visualization
        ...
        for new_node in range(initial_nodes, total_nodes):
            # Position new node
            x, y, z = np.random.uniform(-2, 2), np.random.uniform(-2, 2), np.random.uniform(-2, 2)
            pos[new_node] = (x, y, z)
            new_circle = Circle(radius=0.1).move_to([x * 3, y * 3, z * 3])
